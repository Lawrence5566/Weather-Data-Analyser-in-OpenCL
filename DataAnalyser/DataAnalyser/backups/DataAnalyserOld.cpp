...
int main(int argc, char **argv) {
	...
	try {
		...
		size_t vector_elements = tempArray->size();//number of elements
		size_t vector_size = tempArray->size() * sizeof(int);//size in bytes

		size_t local_size = 2; //custom work group size

		//paddig stuff if we are using manual work group size
		...
		queue.enqueueWriteBuffer(buffer_A, CL_TRUE, 0, vector_size, &tempArray[0]);
